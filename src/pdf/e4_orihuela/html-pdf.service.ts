...
function calculateTextLines(text: string, maxCharsPerLine: number = 37): number {
    if (!text) return 0;
    ...
    for (const line of lines) {
        if (line.length === 0) {
            ...
            if (exactLines <= 1.1) {
                ...
                totalLines += 2.5;
            } else if (exactLines < 3.5) {
                totalLines += 3.5;
            } else if (exactLines < 3) {
                totalLines += 3;
            } else if (exactLines < 4.5) {
                totalLines += 4.5;
            } else {
...
hbs.registerHelper('smartChunk_e4', function(items: any[], options: any) {
    ...
    const tryPlace = (item: any, isContinuation: boolean): { 
        ...
        const textLines = calculateTextLines(text, 37);
        const itemLines = descLength <= 30 ? 1.35 : ROW_BASE_LINES + textLines; 
        ...
        if (avail >= MIN_LINES_TO_DIVIDE) {
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                const testLines = calculateTextLines(test, 37);
            ...
            if (partA) {
                ...
                const partATextLines = calculateTextLines(partA, 37);
                const partALines = descLengthA <= 30 ? 1.35 : ROW_BASE_LINES + partATextLines;
    ...
    while (pending.length > 0) {
        ...
        if (result.result === 'none') {
            ...
            if (result.result === 'none') {
                ...
                const maxChars = maxLinesAvailable * 37;
                const forced = (item.nombre || '').substring(0, maxChars) + '…';
                
                const forcedLength = forced.length;
                const forcedTextLines = calculateTextLines(forced, 37);
                const forcedItemLines = forcedLength <= 30 ? 1.35 : ROW_BASE_LINES + forcedTextLines;
...
hbs.registerHelper('needsSeparateImportantPage_e4', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_e4(items, options);
    ...
    for (const item of lastChunk) {
        ...
        const textLines = calculateTextLines(item.nombre, 37);
        const itemLines = descLength <= 30 ? 1.35 : ROW_BASE_LINES + textLines;
