...
function calcLines4(text: string, maxCPL = 55): number {
  if (!text) return 1;
...
function splitInTwo4(text: string, maxLines: number, cpl: number): [string, string] {
  if (!text) return ['', ''];
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (calcLines4(test, cpl) > maxLines && first.length > 0) {
      cutIdx = i;
      break;
    }
    first = test;
...
function reCalcChunkProd4(chunk: any[]): any[] {
...
function reCalcChunkSvc4(chunk: any[]): any[] {
  const cnt = new Map<number, number>();
...
// PRODUCTOS 
const P4_MAX_FIRST = 22;
const P4_MAX_OTHER = 28;
const P4_FOOTER    = 8;

hbs.registerHelper('licitSmartChunk4', function (rows: any[]) {
  ...
  for (const row of rows) {
    ...
    const max     = isFirst ? P4_MAX_FIRST : P4_MAX_OTHER;
    if (curLines + lines > max && cur.length > 0) { chunks.push([...cur]); cur = []; curLines = 0; }
    cur.push(row); curLines += lines;
  }
  if (cur.length > 0) chunks.push(cur);
  return chunks.map(reCalcChunkProd4);
});

hbs.registerHelper('licitNeedsSeparatePage4', function (rows: any[]) {
  if (!rows || rows.length === 0) return false;
  const chunks = (hbs.helpers as any).licitSmartChunk4(rows);
  if (!chunks.length) return true;
  const last = chunks[chunks.length - 1];
  const lastLines = last.reduce(
    (s: number, r: any) => s + Math.max(1, Math.ceil((r.descripcion?.length ?? 0) / 50)), 0
  );
  const max = chunks.length === 1 ? P4_MAX_FIRST : P4_MAX_OTHER;
  return (lastLines + P4_FOOTER) > max;
});

// SERVICIOS 
const S4_ROW_BASE   = 1;
const S4_MAX_FIRST  = 24;               
const S4_MAX_OTHER  = 36;              
const S4_FOOTER     = 8;
const S4_CPL        = 55;

hbs.registerHelper('licitSmartChunkServicios4', function (rows: any[]) {
  if (!rows || rows.length === 0) return [];
  ...
  const maxForPage = (idx: number) => (idx === 0 ? S4_MAX_FIRST : S4_MAX_OTHER);
  const flush = () => {
    if (curChunk.length) {
  ...
  const processRow = (row: any, isContinuation: boolean = false) => {
    ...
    const descLines = calcLines4(text, S4_CPL);
    const totalLines = S4_ROW_BASE + descLines;
    ...
    const maxDesc = limit - S4_ROW_BASE;
    if (maxDesc <= 0) {
      ...
      curLines = S4_ROW_BASE + calcLines4(truncated, S4_CPL);
      return true;
    }

    const [partA, partB] = splitInTwo4(text, maxDesc, S4_CPL);
    if (!partA) {
    ...
    curLines = S4_ROW_BASE + calcLines4(partA, S4_CPL);
  ...
  while (pendingRows.length > 0) {
    ...
    if (row) processRow(row, row.isContinuation || false);
  }
  flush();

  return chunks.map(chunk => reCalcChunkSvc4(chunk));
});

hbs.registerHelper('licitNeedsSeparatePageServicios4', function (rows: any[]) {
  if (!rows || rows.length === 0) return false;

  const TOTAL_PAGE_LINES = 50;       
  const HEADER_FIRST = 12;           
  const HEADER_OTHER = 2;
  const FOOTER = S4_FOOTER;

  const chunks = (hbs.helpers as any).licitSmartChunkServicios4(rows);
  if (!chunks.length) return true;
  ...
  for (const row of lastChunk) {
    lastTableLines += S4_ROW_BASE + calcLines4(row.descripcion || '', S4_CPL);
  }
...
@Injectable()
export class HtmlLicitacionService4 implements OnModuleInit, OnModuleDestroy {
