...
import { HtmlLicitacionService3 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_3  = 'C. Alejandra Giselle Hernández Islas';
...
export class LicitacionService3 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_3,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-3-licitacion-tecnica-servicios'
        : 'empresa-3-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-3-licitacion-economica-servicios'
      : 'empresa-3-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
        ...
        const precioUnitario = Number(
          (item as any)[`precioFinal${empresa}`] ??
            (
              ...
              (1 + Number((item as any)[`margenPct${empresa}`] ?? 0) / 100)
            ).toFixed(2),
        );
  ...
  private brandByEmpresa(empresa: Emp) {
    return { color: '#e0553c', logo: '../assets/emp3.png' };
  }
