...
function calculateFooterLines(condiciones?: string): number {
    const BASE_FOOTER = 24;
    
    if (!condiciones || !condiciones.trim()) {
        return BASE_FOOTER;
    }
    
    const condicionesLines = calculateTextLines(condiciones, 57);
...
hbs.registerHelper('smartChunk_s6', function(items: any[], options: any) {
...
hbs.registerHelper('needsSeparateImportantPage_s6', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_s6(items, options);
