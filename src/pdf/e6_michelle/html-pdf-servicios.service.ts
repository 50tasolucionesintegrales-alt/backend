...
// Helper para calcular paginación inteligente con división de items largos - SERVICIOS E6
hbs.registerHelper('smartChunkServicios6', function(items: any[]) {
    ...
    const MAX_FIRST_PAGE_LINES = 14;  // ✅ REDUCIDO de 18 a 14 (primera página tiene logo abajo)
    const MAX_OTHER_PAGES_LINES = 30;  // Sin cambios (páginas sin logo)
...
// Helper para saber si necesita página separada para info importante - SERVICIOS E6
hbs.registerHelper('needsSeparatePageServicios6', function(items: any[]) {
    if (!items || items.length === 0) return false;
    
    const chunks = hbs.helpers.smartChunkServicios6(items);
    ...
    // ✅ ULTRA AGRESIVO (como e3/e4): Footer salta casi siempre por el logo grande
    ...
    return availableSpace < 18 || (totalLinesOnLastPage + FOOTER_LINES) > 45;
});
...
@Injectable()
export class HtmlPdfServiciosService6 implements OnModuleInit, OnModuleDestroy {
