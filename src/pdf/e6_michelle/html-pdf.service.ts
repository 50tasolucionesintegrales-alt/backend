...
hbs.registerHelper('smartChunk_e6', function(items: any[], options: any) {
...
hbs.registerHelper('needsSeparateImportantPage_e6', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_e6(items, options);
...
@Injectable()
export class HtmlPdfService6 implements OnModuleInit, OnModuleDestroy {
