...
import { HtmlLicitacionService6 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_6  = 'Michelle Rodríguez Tlaxcalteca';
...
export class LicitacionService6 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_6,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-6-licitacion-tecnica-servicios'
        : 'empresa-6-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-6-licitacion-economica-servicios'
      : 'empresa-6-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#CA8A04', logo: '../assets/emp6.png' };
  }
