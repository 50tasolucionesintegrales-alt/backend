...
// Helper para calcular paginación inteligente con división de items largos - SERVICIOS E5
hbs.registerHelper('smartChunkServicios5', function(items: any[]) {
    ...
    const FOOTER_LINES = 18;  // ✅ AJUSTADO de 22 a 18
...
// Helper para saber si necesita página separada para info importante - SERVICIOS E5
hbs.registerHelper('needsSeparatePageServicios5', function(items: any[]) {
    if (!items || items.length === 0) return false;
    
    const chunks = hbs.helpers.smartChunkServicios5(items);
    ...
    // ✅ BALANCEADO: Si hay más de 15 líneas, footer salta (menos agresivo que 8)
    if (lastPageLines > 15) return true;
    
    // Calcular espacio con margen de seguridad
    const totalPageCapacity = isFirstPage ? 50 : 55;
    const spaceNeeded = totalLinesOnLastPage + FOOTER_LINES;
    const SAFETY_MARGIN = 4;  // Dejar 4 líneas de margen
    
    // Separar si no hay suficiente espacio (con margen)
...
@Injectable()
export class HtmlPdfServiciosService5 implements OnModuleInit, OnModuleDestroy {
