...
import { HtmlLicitacionService5 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_5  = 'Mariana Loeza Hernández';
...
export class LicitacionService5 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_5,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-5-licitacion-tecnica-servicios'
        : 'empresa-5-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-5-licitacion-economica-servicios'
      : 'empresa-5-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#245fa7', logo: '../assets/emp5.png' };
  }
