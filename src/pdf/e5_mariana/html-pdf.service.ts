...
hbs.registerHelper('smartChunk_e5', function(items: any[], options: any) {
    ...
    const tryPlace = (item: any, isContinuation: boolean): { 
        ...
        const textLines = calculateTextLines(text, 50);
        ...
        if (avail >= MIN_LINES_TO_DIVIDE) {
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                const testLines = calculateTextLines(test, 50);
            ...
            if (partA) {
                ...
                const partATextLines = calculateTextLines(partA, 50);
    ...
    while (pending.length > 0) {
        ...
        if (result.result === 'none') {
            ...
            if (result.result === 'none') {
                ...
                const maxChars = maxLinesAvailable * 50;
                const forced = (item.nombre || '').substring(0, maxChars) + '…';
                
                const forcedLength = forced.length;
                const forcedTextLines = calculateTextLines(forced, 50);
...
hbs.registerHelper('needsSeparateImportantPage_e5', function(items: any[], options: any) {
    ...
    const chunks = hbs.helpers.smartChunk_e5(items, options);
    ...
    for (const item of lastChunk) {
        ...
        const textLines = calculateTextLines(item.nombre, 50);
        const itemLines = descLength <= 30 ? 1.35 : ROW_BASE_LINES + textLines;
