...
function calculateTextLines(text: string, maxCharsPerLine: number = 56): number {
    if (!text) return 0;
...
const MAX_FIRST_PAGE_LINES = 43;
const MAX_OTHER_PAGES_LINES = 53;
const ROW_BASE_LINES = 1;
const MAX_PAGE_CAPACITY = 57;
...
function calculateFooterLines(condiciones?: string): number {
    ...
    const condicionesLines = calculateTextLines(condiciones, 55);
...
hbs.registerHelper('smartChunk_e8', function(items: any[], options: any) {
    ...
    const tryPlace = (item: any, isContinuation: boolean): { 
        ...
        const textLines = calculateTextLines(text, 56);
        const itemLines = descLength <= 30 ? 1.75 : ROW_BASE_LINES + textLines;
        ...
        if (avail >= MIN_LINES_TO_DIVIDE) {
            ...
            for (let i = 0; i < words.length; i++) {
                ...
                const testLines = calculateTextLines(test, 56);
            ...
            if (partA) {
                ...
                const partATextLines = calculateTextLines(partA, 56);
                const partALines = descLengthA <= 30 ? 1.75 : ROW_BASE_LINES + partATextLines;
    ...
    while (pending.length > 0) {
        ...
        if (result.result === 'none') {
            ...
            if (result.result === 'none') {
                ...
                const maxChars = maxLinesAvailable * 56;
                const forced = (item.nombre || '').substring(0, maxChars) + '…';
                
                const forcedLength = forced.length;
                const forcedTextLines = calculateTextLines(forced, 56);
                const forcedItemLines = forcedLength <= 30 ? 1.75 : ROW_BASE_LINES + forcedTextLines;
...
hbs.registerHelper('needsSeparateImportantPage_e8', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_e8(items, options);
    ...
    for (const item of lastChunk) {
        ...
        const textLines = calculateTextLines(item.nombre, 56);
        const itemLines = descLength <= 30 ? 1.75 : ROW_BASE_LINES + textLines;
...
@Injectable()
export class HtmlPdfService8 implements OnModuleInit, OnModuleDestroy {
