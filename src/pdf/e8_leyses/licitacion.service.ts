...
import { HtmlLicitacionService8 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_8  = 'Leyses';
...
export class LicitacionService8 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_8,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-8-licitacion-tecnica-servicios'
        : 'empresa-8-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-8-licitacion-economica-servicios'
      : 'empresa-8-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#111111', logo: '../assets/emp8.png' };
  }
