...
// Helper para calcular paginación inteligente con división de items largos - SERVICIOS E8
hbs.registerHelper('smartChunkServicios8', function(items: any[]) {
    ...
    const MAX_FIRST_PAGE_LINES = 24;   // ✅ AUMENTADO de 18 a 24
    const MAX_OTHER_PAGES_LINES = 38;  // ✅ AUMENTADO de 30 a 38
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (isFirstPage && currentChunk.length === 0 && itemLines > MAX_FIRST_PAGE_LINES) {
            ...
            for (let p = 0; p < parts.length; p++) {
                ...
                // Si no es la primera parte Y no cabe con el contenido actual, nueva página
                if (p > 0 && currentLines + partItemLines > partMaxLines && currentChunk.length > 0) {
        ...
        // OTRAS PÁGINAS: dividir si NO CABE
            ...
            for (let p = 0; p < parts.length; p++) {
                ...
                // Si no es la primera parte Y no cabe con el contenido actual, nueva página
                if (p > 0 && currentLines + partItemLines > MAX_OTHER_PAGES_LINES && currentChunk.length > 0) {
                    chunks.push([...currentChunk]);
                    currentChunk = [];
                    currentLines = 0;
                }
        ...
        // ITEM NORMAL: cabe completo
            ...
            // ✅ CORREGIDO: Verificar si cabe en página actual
            if (currentLines + itemLines > currentMaxLines && currentChunk.length > 0) {
                chunks.push([...currentChunk]);
                currentChunk = [];
                currentLines = 0;
            }
...
// Helper para saber si necesita página separada para info importante - SERVICIOS E8
hbs.registerHelper('needsSeparatePageServicios8', function(items: any[]) {
    if (!items || items.length === 0) return false;
    
    const chunks = hbs.helpers.smartChunkServicios8(items);
    ...
    // ✅ EXTREMADAMENTE PERMISIVO: El footer casi nunca salta
    if (lastPageLines > 28) return true;  // Aumentado de 20 a 28
    
    // Calcular espacio SIN margen de seguridad
    const totalPageCapacity = isFirstPage ? 50 : 55;
    const spaceNeeded = totalLinesOnLastPage + FOOTER_LINES;
    const SAFETY_MARGIN = 0;  // SIN margen - solo saltar si realmente NO CABE
    
    // Separar SOLO si matemáticamente no cabe
    return spaceNeeded > (totalPageCapacity - SAFETY_MARGIN);
});
...
@Injectable()
export class HtmlPdfServiciosService8 implements OnModuleInit, OnModuleDestroy {
