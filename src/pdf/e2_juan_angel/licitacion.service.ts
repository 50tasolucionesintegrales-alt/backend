...
import { HtmlLicitacionService2 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_2  = 'C. Juan Angel Bazan Gonzalez';
...
export class LicitacionService2 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa: FIRMANTE_EMPRESA_2,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-2-licitacion-tecnica-servicios'
        : 'empresa-2-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-2-licitacion-economica-servicios'
      : 'empresa-2-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
