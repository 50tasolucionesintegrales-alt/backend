...
import { HtmlLicitacionService7 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_7  = 'Mariana Loeza Hernández';
...
export class LicitacionService7 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_7,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-7-licitacion-tecnica-servicios'
        : 'empresa-7-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-7-licitacion-economica-servicios'
      : 'empresa-7-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#5b3ca5', logo: '../assets/emp7.png' };
  }
