...
function calcLines7(text: string, maxCPL = 55): number {
  if (!text) return 1;
...
function splitInTwo7(text: string, maxLines: number, cpl: number): [string, string] {
  if (!text) return ['', ''];
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (calcLines7(test, cpl) > maxLines && first.length > 0) {
      cutIdx = i;
      break;
    }
    first = test;
...
function reCalcChunkProd7(chunk: any[]): any[] {
...
function reCalcChunkSvc7(chunk: any[]): any[] {
  const cnt = new Map<number, number>();
...
// ─────────────────────────────────────────────────────────────────
// EMPRESA 7 — padding-top: 30mm, padding-bottom: 30mm
// Disponible: 279.4 - 30 - 30 = 219.4mm
// ─────────────────────────────────────────────────────────────────

// PRODUCTOS
const P7_MAX_FIRST = 18;
const P7_MAX_OTHER = 24;
const P7_FOOTER    = 8;

hbs.registerHelper('licitSmartChunk7', function (rows: any[]) {
  ...
  for (const row of rows) {
    ...
    const max     = isFirst ? P7_MAX_FIRST : P7_MAX_OTHER;
    if (curLines + lines > max && cur.length > 0) { chunks.push([...cur]); cur = []; curLines = 0; }
    cur.push(row); curLines += lines;
  }
  if (cur.length > 0) chunks.push(cur);
  return chunks.map(reCalcChunkProd7);
});

hbs.registerHelper('licitNeedsSeparatePage7', function (rows: any[]) {
  if (!rows || rows.length === 0) return false;
  const chunks = (hbs.helpers as any).licitSmartChunk7(rows);
  if (!chunks.length) return true;
  const last = chunks[chunks.length - 1];
  const lastLines = last.reduce(
    (s: number, r: any) => s + Math.max(1, Math.ceil((r.descripcion?.length ?? 0) / 50)), 0
  );
  const max = chunks.length === 1 ? P7_MAX_FIRST : P7_MAX_OTHER;
  return (lastLines + P7_FOOTER) > max;
});

// SERVICIOS
const S7_MAX_FIRST  = 18;
const S7_MAX_OTHER  = 26;
const S7_FOOTER     = 7;
const S7_ROW_BASE   = 1;
const S7_CPL        = 55;

hbs.registerHelper('licitSmartChunkServicios7', function (rows: any[]) {
  if (!rows || rows.length === 0) return [];
  ...
  const getMax   = () => (chunks.length === 0 ? S7_MAX_FIRST : S7_MAX_OTHER);
  ...
  function addDesc(originalRow: any, desc: string, isCont: boolean): void {
    if (!desc) return;
    const dLines   = calcLines7(desc, S7_CPL);
    const rowLines = S7_ROW_BASE + dLines;
    ...
    const maxDescLines = avail - S7_ROW_BASE;
    if (maxDescLines >= 2) {
      const [partA, partB] = splitInTwo7(desc, maxDescLines, S7_CPL);
      if (partA && partB) {
    ...
    flush();
    const freshDLines = calcLines7(desc, S7_CPL);
    const freshMax    = getMax() - S7_ROW_BASE;

    if (freshDLines <= freshMax) {
      cur.push({ ...originalRow, descripcion: desc, isContinuation: isCont });
      curLines += S7_ROW_BASE + freshDLines;
    } else {
      const [partA, partB] = splitInTwo7(desc, freshMax, S7_CPL);
      cur.push({ ...originalRow, descripcion: partA || desc, isContinuation: isCont });
      curLines += S7_ROW_BASE + calcLines7(partA || desc, S7_CPL);
  ...
  flush();
  return chunks.map(reCalcChunkSvc7);
});

hbs.registerHelper('licitNeedsSeparatePageServicios7', function (rows: any[]) {
  if (!rows || rows.length === 0) return false;
  const chunks = (hbs.helpers as any).licitSmartChunkServicios7(rows);
  if (!chunks.length) return true;
  const last      = chunks[chunks.length - 1];
  const lastLines = last.reduce(
    (s: number, r: any) => s + S7_ROW_BASE + calcLines7(r.descripcion ?? '', S7_CPL), 0
  );
  const max = chunks.length === 1 ? S7_MAX_FIRST : S7_MAX_OTHER;
  return (lastLines + S7_FOOTER) > max;
});
...
@Injectable()
export class HtmlLicitacionService7 implements OnModuleInit, OnModuleDestroy {
