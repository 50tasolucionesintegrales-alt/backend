...
import { HtmlLicitacionService1 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_1  = 'Soluciones Goltech S.A. de C.V.';
...
export class LicitacionService1 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      // Constante — no viene del input del usuario
      firmanteEmpresa:    FIRMANTE_EMPRESA_1,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-1-licitacion-tecnica-servicios'
        : 'empresa-1-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-1-licitacion-economica-servicios'
      : 'empresa-1-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
