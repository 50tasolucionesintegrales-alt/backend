...
const MAX_FIRST_PAGE_LINES = 38;
const MAX_OTHER_PAGES_LINES = 44;
...
hbs.registerHelper('smartChunk_s1', function(items: any[], options: any) {
    ...
    const tryPlace = (item: any, isContinuation: boolean): { 
        ...
        const itemLines = ROW_BASE_LINES + textLines;
        ...
        if (itemLines <= avail) {
            const headerLines = pageIdx === 0 ? 14 : 2;
        ...
        if (avail >= MIN_LINES_TO_DIVIDE) {
            ...
            if (partA) {
                ...
                const partALines = ROW_BASE_LINES + partATextLines;
    ...
    while (pending.length > 0) {
        ...
        if (result.result === 'none') {
            ...
            if (result.result === 'none') {
                ...
                const forcedItemLines = ROW_BASE_LINES + forcedTextLines;
...
hbs.registerHelper('needsSeparateImportantPage_s1', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_s1(items, options);
    ...
    const headerLines = isFirstPage ? 14 : 2;
