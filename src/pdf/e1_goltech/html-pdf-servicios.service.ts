...
hbs.registerHelper('smartChunkServicios', function(items: any[]) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        // Verificar si footer cabe cuando es último item
        if (isLastItem) {
        ...
        // ✅ PRIMERA PÁGINA: dividir si NO CABE (usa límite real)
        if (isFirstPage && currentChunk.length === 0 && itemLines > MAX_FIRST_PAGE_LINES) {
            // División inteligente: primera parte = lo que cabe, resto = todo junto
            const parts: string[] = [];
            const text = item.nombre;
            const words = text.split(' ');
        ...
        // ✅ OTRAS PÁGINAS: dividir si NO CABE (usa límite real)
