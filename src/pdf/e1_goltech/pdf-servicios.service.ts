...
@Injectable()
export class PdfServiciosService1 {
  constructor(private readonly html: HtmlPdfServiciosService1) { }
  ...
  private computeTotals(quote: Quote, m: Emp) {
    const items = quote.items.map((it) => {
      const descripcion = it.service?.descripcion ?? it.service?.nombre ?? '—'; // ✅ Solo servicios
