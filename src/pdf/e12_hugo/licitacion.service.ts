...
import { HtmlLicitacionService12 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_12  = 'Hugo Rodríguez Hernández';
...
export class LicitacionService12 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_12,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-12-licitacion-tecnica-servicios'
        : 'empresa-12-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-12-licitacion-economica-servicios'
      : 'empresa-12-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#143C23', logo: '../assets/emp12.png' };
  }
