...
const FIRMA_BY_EMPRESA: Record<Emp, string> = {
  12: 'assets/firma_emp12.png',
};

const FIRMA_FALLBACK = 'assets/firma.png';

@Injectable()
export class PdfServiciosService12 {
  constructor(private readonly html: HtmlPdfServiciosService12) { }
  ...
  async generateOneBuffer(
    ...
    const lugar = meta.lugar?.trim() || 'Mineral de la Reforma, Hgo';
    const presente = (meta.presente?.trim() || 'Presente.');
    const condiciones = meta.condiciones ?? '';
    const incluirFirma = meta.incluirFirma ?? true;
    const firmanteNombre = meta.firmanteNombre?.trim() || 'C. Hugo Rodríguez Hernández';
    ...
    const template = `empresa-${empresa}-servicios`;
  ...
  private brandByEmpresa(empresa: Emp) {
    const map: Record<Emp, { color: string; logo: string }> = {
      12: { color: '#2c3e50', logo: '../assets/emp12.png' },
    };
    return map[empresa];
  }
  ...
  private computeTotals(quote: Quote, m: Emp) {
    const items = quote.items.map((it) => {
      const descripcion = it.service?.descripcion ?? it.service?.nombre ?? '—';
      
      const unidad = (it as any).unidad ?? 'Servicio';
