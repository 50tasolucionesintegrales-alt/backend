...
function calculateFooterLines(condiciones?: string): number {
    ...
    const condicionesLines = calculateTextLines(condiciones, 38);
...
hbs.registerHelper('smartChunk_e12', function(items: any[], options: any) {
...
hbs.registerHelper('needsSeparateImportantPage_e12', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_e12(items, options);
