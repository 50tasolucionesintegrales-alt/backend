...
function calcLines10(text: string, maxCPL = 55): number {
  if (!text) return 1;
...
function splitInTwo10(text: string, maxLines: number, cpl: number): [string, string] {
  if (!text) return ['', ''];
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (calcLines10(test, cpl) > maxLines && first.length > 0) {
      cutIdx = i;
      break;
    }
    first = test;
...
function reCalcChunkProd10(chunk: any[]): any[] {
...
function reCalcChunkSvc10(chunk: any[]): any[] {
  const cnt = new Map<number, number>();
...
// ─────────────────────────────────────────────────────────────────
// EMPRESA 10 — padding-top: 50mm, padding-bottom: 30mm
// Disponible: 279.4 - 50 - 30 = 199.4mm
// ─────────────────────────────────────────────────────────────────

// PRODUCTOS
const P10_MAX_FIRST = 18;
const P10_MAX_OTHER = 24;
const P10_FOOTER    = 6;

hbs.registerHelper('licitSmartChunk10', function (rows: any[]) {
  ...
  for (const row of rows) {
    ...
    const max     = isFirst ? P10_MAX_FIRST : P10_MAX_OTHER;
    if (curLines + lines > max && cur.length > 0) { chunks.push([...cur]); cur = []; curLines = 0; }
    cur.push(row); curLines += lines;
  }
  if (cur.length > 0) chunks.push(cur);
  return chunks.map(reCalcChunkProd10);
});

hbs.registerHelper('licitNeedsSeparatePage10', function (rows: any[]) {
  if (!rows || rows.length === 0) return false;
  const chunks = (hbs.helpers as any).licitSmartChunk10(rows);
  if (!chunks.length) return true;
  const last = chunks[chunks.length - 1];
  const lastLines = last.reduce(
    (s: number, r: any) => s + Math.max(1, Math.ceil((r.descripcion?.length ?? 0) / 50)), 0
  );
  const max = chunks.length === 1 ? P10_MAX_FIRST : P10_MAX_OTHER;
  return (lastLines + P10_FOOTER) > max;
});

// SERVICIOS
const S10_MAX_FIRST  = 18;
const S10_MAX_OTHER  = 24;
const S10_FOOTER     = 6;
const S10_ROW_BASE   = 1;
const S10_CPL        = 55;

hbs.registerHelper('licitSmartChunkServicios10', function (rows: any[]) {
  if (!rows || rows.length === 0) return [];
  ...
  const getMax   = () => (chunks.length === 0 ? S10_MAX_FIRST : S10_MAX_OTHER);
  ...
  function addDesc(originalRow: any, desc: string, isCont: boolean): void {
    if (!desc) return;
    const dLines   = calcLines10(desc, S10_CPL);
    const rowLines = S10_ROW_BASE + dLines;
    ...
    const maxDescLines = avail - S10_ROW_BASE;
    if (maxDescLines >= 2) {
      const [partA, partB] = splitInTwo10(desc, maxDescLines, S10_CPL);
      if (partA && partB) {
    ...
    flush();
    const freshDLines = calcLines10(desc, S10_CPL);
    const freshMax    = getMax() - S10_ROW_BASE;

    if (freshDLines <= freshMax) {
      cur.push({ ...originalRow, descripcion: desc, isContinuation: isCont });
      curLines += S10_ROW_BASE + freshDLines;
    } else {
      const [partA, partB] = splitInTwo10(desc, freshMax, S10_CPL);
      cur.push({ ...originalRow, descripcion: partA || desc, isContinuation: isCont });
      curLines += S10_ROW_BASE + calcLines10(partA || desc, S10_CPL);
  ...
  flush();
  return chunks.map(reCalcChunkSvc10);
});

hbs.registerHelper('licitNeedsSeparatePageServicios10', function (rows: any[]) {
  if (!rows || rows.length === 0) return false;
  const chunks = (hbs.helpers as any).licitSmartChunkServicios10(rows);
  if (!chunks.length) return true;
  const last      = chunks[chunks.length - 1];
  const lastLines = last.reduce(
    (s: number, r: any) => s + S10_ROW_BASE + calcLines10(r.descripcion ?? '', S10_CPL), 0
  );
  const max = chunks.length === 1 ? S10_MAX_FIRST : S10_MAX_OTHER;
  return (lastLines + S10_FOOTER) > max;
});
...
@Injectable()
export class HtmlLicitacionService10 implements OnModuleInit, OnModuleDestroy {
