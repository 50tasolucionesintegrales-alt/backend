...
// Helper para calcular paginación inteligente con división de items largos - SERVICIOS E10
hbs.registerHelper('smartChunkServicios10', function(items: any[]) {
    ...
    const MAX_FIRST_PAGE_LINES = 10;
    const MAX_OTHER_PAGES_LINES = 18;
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        let descLines = calculateTextLines(item.nombre, 50);
        ...
        if (isFirstPage && currentChunk.length === 0 && itemLines > MAX_FIRST_PAGE_LINES) {
            ...
            for (let p = 0; p < parts.length; p++) {
                const partDescLines = calculateTextLines(parts[p], 50);
...
// Helper para saber si necesita página separada para info importante - SERVICIOS E10
hbs.registerHelper('needsSeparatePageServicios10', function(items: any[]) {
    ...
    if (lastPageLines > 10) return true;
    
    const availableSpace = (isFirstPage ? 35 : 42) - totalLinesOnLastPage;
    return availableSpace < 16 || (totalLinesOnLastPage + FOOTER_LINES) > 35;
});
...
@Injectable()
export class HtmlPdfServiciosService10 implements OnModuleInit, OnModuleDestroy {
