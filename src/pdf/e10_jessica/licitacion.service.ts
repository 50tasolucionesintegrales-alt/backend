...
import { HtmlLicitacionService10 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_10  = 'Jessica Rabadán Castro';
...
export class LicitacionService10 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_10,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-10-licitacion-tecnica-servicios'
        : 'empresa-10-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-10-licitacion-economica-servicios'
      : 'empresa-10-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#000000', logo: '../assets/emp10.png' };
  }
