...
import { HtmlLicitacionService11 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_11  = 'Grupo Álamo Alto';
...
export class LicitacionService11 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_11,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-11-licitacion-tecnica-servicios'
        : 'empresa-11-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-11-licitacion-economica-servicios'
      : 'empresa-11-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#93CEBC', logo: '../assets/emp11.png' };
  }
