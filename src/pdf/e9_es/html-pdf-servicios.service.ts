...
const MAX_FIRST_PAGE_LINES = 19;
const MAX_OTHER_PAGES_LINES = 25;
...
hbs.registerHelper('smartChunk_s9', function(items: any[], options: any) {
    ...
    const tryPlace = (item: any, isContinuation: boolean): { 
        ...
        const itemLines = ROW_BASE_LINES + calculateTextLines(text, 52);
        ...
        if (avail >= MIN_LINES_TO_DIVIDE) {
            ...
            if (partA) {
                const partALines = ROW_BASE_LINES + calculateTextLines(partA, 52);
    ...
    while (pending.length > 0) {
        ...
        if (result.result === 'none') {
            ...
            if (result.result === 'none') {
                ...
                curLines = ROW_BASE_LINES + calculateTextLines(forced, 52);
                continue;
            }
        }
...
hbs.registerHelper('needsSeparateImportantPage_s9', function(items: any[], options: any) {
    if (!items || items.length === 0) return false;

    const condiciones = options?.data?.root?.condiciones || '';
    const footerLines = calculateFooterLines(condiciones);

    const chunks = hbs.helpers.smartChunk_s9(items, options);
    ...
    for (const item of lastChunk) {
        lastTableLines += ROW_BASE_LINES + calculateTextLines(item.nombre, 52);
    }
