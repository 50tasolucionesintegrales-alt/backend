...
import { HtmlLicitacionService9 } from './html-licitacion.service';
...
const FIRMANTE_EMPRESA_9  = 'Eduardo Suárez';
...
export class LicitacionService9 {
  ...
  async generateBuffer(quote: Quote, dto: GenerateLicitacionPdfDto): Promise<Buffer> {
    ...
    const baseData = {
      ...
      firmanteEmpresa:  FIRMANTE_EMPRESA_9,
    };

    if (dto.tipo === 'tecnica') {
      const data = { ...baseData, tipoDocumento: 'Propuesta Técnica' };
      const template = esServicios
        ? 'empresa-9-licitacion-tecnica-servicios'
        : 'empresa-9-licitacion-tecnica';
    ...
    const template = esServicios
      ? 'empresa-9-licitacion-economica-servicios'
      : 'empresa-9-licitacion-economica';
    return this.html.renderToPdf(template, data);
  }
  ...
  private brandByEmpresa(_empresa: Emp) {
    return { color: '#195a60', logo: '../assets/emp9.png' };
  }
